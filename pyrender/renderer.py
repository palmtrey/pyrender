...
class Renderer(object):
    ...
    def render(self, scene, flags, seg_node_map=None):
        ...
        if not bool(flags & RenderFlags.DEPTH_ONLY or flags & RenderFlags.SEG):
            for ln in scene.light_nodes:
                take_pass = False
                if isinstance(ln.light, DirectionalLight) and bool(
                    flags & RenderFlags.SHADOWS_DIRECTIONAL
                ):
                    take_pass = True
                elif isinstance(ln.light, SpotLight) and bool(
                    flags & RenderFlags.SHADOWS_SPOT
                ):
                    take_pass = True
                elif isinstance(ln.light, PointLight) and bool(
    ...
    def read_depth_buf(self):
        ...
        if z_far is None:
            ...
            depth_im[noninf] = (2.0 * z_near * z_far) / (
                z_far + z_near - depth_im[noninf] * (z_far - z_near)
            )
        ...
        for node in self._sorted_mesh_nodes(scene):
            ...
            for primitive in mesh.primitives:
                ...
                if not (
                    ...
                    or flags & RenderFlags.FLAT
                    or flags & RenderFlags.SEG
                ):
    ...
    def _bind_and_draw_primitive(self, primitive, pose, program, flags):
        ...
        if not (flags & RenderFlags.DEPTH_ONLY or flags & RenderFlags.SEG):
            ...
            if tf & TexFlags.NORMAL:
                self._bind_texture(
                    material.normalTexture, "material.normal_texture", program
                )
            if tf & TexFlags.OCCLUSION:
                self._bind_texture(
                    material.occlusionTexture, "material.occlusion_texture", program
                )
            if tf & TexFlags.EMISSIVE:
                self._bind_texture(
                    material.emissiveTexture, "material.emissive_texture", program
                )
            if tf & TexFlags.BASE_COLOR:
                self._bind_texture(
                    material.baseColorTexture, "material.base_color_texture", program
                )
            ...
            if tf & TexFlags.DIFFUSE:
                self._bind_texture(
                    material.diffuseTexture, "material.diffuse_texture", program
                )
            ...
            if isinstance(material, MetallicRoughnessMaterial):
                program.set_uniform(
                    b.format("base_color_factor"), material.baseColorFactor
                )
                program.set_uniform(
                    b.format("metallic_factor"), material.metallicFactor
                )
                program.set_uniform(
                    b.format("roughness_factor"), material.roughnessFactor
                )
            elif isinstance(material, SpecularGlossinessMaterial):
                program.set_uniform(b.format("diffuse_factor"), material.diffuseFactor)
                program.set_uniform(
                    b.format("specular_factor"), material.specularFactor
                )
                program.set_uniform(
                    b.format("glossiness_factor"), material.glossinessFactor
                )
    ...
    def _bind_lighting(self, scene, program, node, flags):
        ...
        for n in light_nodes:
            ...
            if isinstance(light, PointLight):
                ...
                las = 1.0 / max(
                    0.001, np.cos(light.innerConeAngle) - np.cos(light.outerConeAngle)
                )
    ...
    def _get_primitive_program(self, primitive, flags, program_flags):
        ...
        if (
            ...
            program_flags & (ProgramFlags.VERTEX_NORMALS | ProgramFlags.FACE_NORMALS)
        ):
    ...
    def _resize_image(self, value, antialias=False):
        ...
        size = (
            self.viewport_width // self.dpscale,
